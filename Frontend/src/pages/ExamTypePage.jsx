    ...
    <div className="w-full flex flex-col ml-auto mr-auto">
      ...
      <div className=" flex flex-col sm:mt-5 md:mt-10 gap-4 items-center justify-center">
        ...
        <div className=" flex mt-20 gap-4 items-center justify-center">
          <h1 className=" sm:text-xl  md:text-2xl text-black font-bold">
          ...
          {/* listitem */}
          <div className={`mt-[30px]  py-10 transition-all duration-300 w-[80%] ml-16 lg:w-[70%] bg-gray-300 border-2 border-black flex flex-col gap-6 md:gap-8 items-center
        ${isOpen ? "lg:ml-50 duration-300" : "lg:ml-20 duration-300"}`} >
            {data.map((item, key) => (
              <div key={key} className="border rounded border-black sm:text-sm md:text-xl p-4 w-[90%]">
                ...
                <Link to="/subjects" state={{ examType: item.name }}>
                  <button className="font-bold relative group bg-black px-6 py-3 text-white rounded-xl cursor-pointer border-2 border-transparent hover:bg-white hover:text-black hover:border-black transition-all duration-500 md:w-16 flex justify-center">
                    {item.name}
                  </button>
                </Link>
